...
if __name__ == "__main__":
    ...
    # HL *= 1e-24 * ase.units.m**2 * ase.units.kg / ase.units.J / mass
    # TL *= 1e-24 * ase.units.m**2 * ase.units.kg / ase.units.J / mass
    # KC *= 1e-24 * ase.units.m**2 * ase.units.kg / ase.units.J / mass
    # VLC *= 1e-24 * ase.units.m**2 * ase.units.kg / ase.units.J / mass
    # VCR *= 1e-24 * ase.units.m**2 * ase.units.kg / ase.units.J / mass
    ...
    inc_omega = np.linspace(
        0.1, max(omegaL.max(), omegaR.max()) * 1.01, num=NPOINTS, endpoint=True
    )
    ...
    for iomega, omega in enumerate(tqdm.tqdm(inc_omega, dynamic_ncols=True)):
        ...
        tmp =  np.where(np.diag(ILretp).real!=0)[0]
        print(tmp)
        ...
        # tau = la.solve(URretp, GRLret) @ QL @ ULretp
        # tRL1 = VRretp12 @ tau @ np.sqrt(VLadvm_tilde)
    ...
    # fig = plt.figure(figsize=(8.0, 10.0))
    # plt.subplot(221)
    plt.plot(inc_omega, NLm, label=r"$N_{L-}$")
    # plt.xlabel("$\omega\;(\mathrm{rad/ps})$")
    # plt.ylabel(r"$N_{L-}(\omega)$")
    # plt.subplot(222)
    plt.plot(inc_omega, NRm, label=r"$N_{R-}$")
    # plt.xlabel("$\omega\;(\mathrm{rad/ps})$")
    # plt.ylabel(r"$N_{R-}(\omega)$")
    # plt.subplot(223)
    plt.plot(inc_omega, NLp, label=r"$N_{L+}$")
    # plt.xlabel("$\omega\;(\mathrm{rad/ps})$")
    # plt.ylabel(r"$N_{L+}(\omega)$")
    # plt.subplot(224)
    plt.plot(inc_omega, NRp, label=r"$N_{R+}$")
    # plt.xlabel("$\omega\;(\mathrm{rad/ps})$")
    # plt.ylabel(r"$N_{R+}(\omega)$")
    ...
    plt.savefig(os.path.join(path_directory, "transmission-pure_defect-total.png"), dpi=600)
    plt.show()
