...
def main():
    ...
    if family==2:
        ...
        for ii, freq in enumerate(frequencies):
            # set_trace()
            idx_ir = (ii > dim_sum - 1).sum()
            if ii in dim_sum-1 and idx_ir>=int(q_rot/2) - 1:
                ax.plot(np.array(distances), freq, label=labels[int(abs(idx_ir-q_rot/2+1))], color=color[int(abs(idx_ir-q_rot/2+1))])
            else:
                ax.plot(np.array(distances), freq, color=color[int(abs(idx_ir-q_rot/2+1))])
