...
from utilities import divide_irreps, divide_over_irreps, get_adapted_matrix_multiq, get_adapted_matrix, get_freqAndeigvec_from_qp
    ...
    def orthogonalize(values, vectors, adapteds, k_adapteds, dimensions):
        ...
        for i_m, m in enumerate(group_masks):
            ...
            try:
                ...
                # plt.savefig("phonon_raw", dpi=600)
                # plt.show()

                DictParams = {"qpoints": qp, "nrot": nrot, "order": order_ops, "family": family, "a": aL}
                D = phonon.get_dynamical_matrix_at_q(qz)
                freqs, vecs = get_freqAndeigvec_from_qp(D, DictParams, num_atom, matrices)

                res1 = np.abs(freqs - omega)
                min_position = np.unravel_index(np.argmin(res1), res1.shape)
                print(vecs[min_position[0], min_position[1]])
                set_trace()
                continue
