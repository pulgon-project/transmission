...
if __name__ == "__main__":
    ...
    for i_line, path_directory in enumerate(path_directory0):
        ...
        path_savefig = "figure_tri_nontri_transmision"
        ######################### projector
        ...
        linestyle = ["-", ":"]
        label_sum = ["Tri_Irreps_sum", "NonTri_Irreps_sum"]
        ...
        plt.plot(np.array(inc_omega), NLp_sum, linestyle=linestyle[i_line], label=label_sum[i_line], color="pink")
        ...
        for ii, freq in enumerate(NLp_irreps):
            plt.plot(np.array(inc_omega), freq, linestyle=linestyle[i_line], label=labels[ii], color=color[ii])
