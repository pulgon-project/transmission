...
path_1 = os.path.join(path_0, '%s-u1-%d-defect-%s-%s-%d' % (chariality1,num_cell1, element, sym1,ind1))
path_2 = os.path.join(path_0, '%s-u1-%d-defect-%s-%s-%d' % (chariality2,num_cell2, element, sym2,ind2))
path_3 = os.path.join(path_0, '%s-u1-%d-defect-%s-%s-%d' % (chariality3,num_cell3, element, sym3,ind3))
...
path_savefig = os.path.join(path_0, "Irreps-%s-%s-%d-%s-%d-%s-%d" % (element,sym1, ind1, sym2, ind2, sym3, ind3))

path_trans1 = os.path.join(path_1, "transmission_irreps.npz")
path_trans2 = os.path.join(path_2, "transmission_irreps.npz")
path_trans3 = os.path.join(path_3, "transmission_irreps.npz")


data1 = np.load(path_trans1)
data2 = np.load(path_trans2)
data3 = np.load(path_trans3)
...
for ii, _ in enumerate(NLp_irreps1):
    ...
    axes[ii//2,np.mod(ii,2)].plot(inc_omega, NLp_irreps3[ii], label=r"%s - |m|=%d" % (label3, ii), linestyle=linestyles[2], color=point_colors[2], marker=markers[3], markersize=3)
