...
def read_FORCE_CONSTANTS(filename):
    ...
    """
    with lzma.open(filename, mode="r") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "qa", help="Wave vector along the first in-plane direction")
    parser.add_argument(
        "qb", help="Wave vector along the second in-plane direction")
    ...
    for u in ucells:
        pmasses[u[2]] = sum(
            [[math.sqrt(i)] * 3 for i in atoms.get_masses()[ucells[u]]], [])
    ...
    for k in onedfcsL:
        onedfcsL[k] *= (
            1e-24 * ase.units.m**2 * ase.units.kg / ase.units.J / smassesL
        )  # (rad / ps)**2
    massesR = pmasses[startR] * len(motifR)
    smassesR = np.outer(massesR, massesR)
    for k in onedfcsR:
        onedfcsR[k] *= (
            1e-24 * ase.units.m**2 * ase.units.kg / ase.units.J / smassesR
        )  # (rad / ps)**2
    ...
    for k in onedfcs:
        ...
        onedfcs[k] *= (
            1e-24 * ase.units.m**2 * ase.units.kg / ase.units.J / smasses
        )  # (rad / ps)**2
    ...
    if True:
        ...
        plt.plot(qvec, omegaL)
        plt.xlabel("$q_z\cdot L$")
        plt.ylabel("$\omega\;(\mathrm{rad/ps})$")
        plt.title("Left lead")
        plt.tight_layout()
    ...
    for iomega, omega in enumerate(tqdm.tqdm(inc_omega)):
        ...
        # yapf: enable
        Gret = la.pinv(
            en * np.eye(H_pr.shape[0], dtype=np.complex128) - H_pr, rcond=1e-3)

        # Compute the total transmission.
        GammaL = 1.j * TL.conj().T @ (
            la.solve(inv_gLretm, TL) - la.solve(inv_gLadvm, TL))
        GammaR = 1.j * TR @ (
            la.solve(inv_gRretp,
                     TR.conj().T) - la.solve(inv_gRadvp,
                                             TR.conj().T))
        GLRret = Gret[:HL_pr.shape[0], -HR_pr.shape[1]:]
        GRLret = Gret[-HR_pr.shape[0]:, :HL_pr.shape[1]]

        trans[iomega] = np.trace(
            GRLret @ (GRLret @ GammaL.conj().T).conj().T @ GammaR).real
        ...
        if not np.isclose(trans_check[iomega], trans[iomega], atol=1.):
    ...
    if True and NPOINTS == 50:
        ...
        for i in range(5):
            for j in range(10):
                ...
                if matrices[k].size > 0:
                    im = axs[i, j].matshow(matrices[k], vmin=0., vmax=1.)
                    axs[i, j].set_xticks([])
                    axs[i, j].set_yticks([])
                else:
                    axs[i, j].axis("off")
                axs[i, j].text(
                    ...
                    fontdict=dict(
                        size=fsize / 1.2, color="red", weight="bold"))
        ...
        # plt.colorbar()
        plt.tight_layout()
        plt.subplots_adjust(hspace=1e-3, wspace=1e-3, right=.9)
